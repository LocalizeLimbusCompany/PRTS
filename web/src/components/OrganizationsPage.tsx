...
export function OrganizationsPage() {
  ...
  return (
    <AppShell
      ...
      actions={
        canCreateOrganization ? (
          <Link to="/organizations/new" className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-3 text-sm font-semibold text-white shadow-sm shadow-slate-900/20">
    ...
    >
      <div className="grid gap-1 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-[28px] border border-slate-200 bg-white/95 p-1.5 shadow-[0_30px_60px_-40px_rgba(15,23,42,0.3)]">
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {organizations.map((org) => (
              ...
              >
                ...
                <p className="mt-2 line-clamp-3 text-sm leading-tight text-slate-500">{org.description || t('organizations.emptyDesc')}</p>
        ...
        <aside className="rounded-[28px] border border-slate-200 bg-slate-950 p-1.5 text-white shadow-[0_30px_70px_-45px_rgba(15,23,42,0.7)]">
          <div className="mt-5 text-2xl font-semibold tracking-tight">{t('organizations.sideTitle')}</div>
          <p className="mt-2 text-sm leading-tight text-slate-300">{t('organizations.sideBody')}</p>

          {!canCreateOrganization ? (
            <div className="mt-3 rounded-[24px] border border-amber-400/20 bg-amber-400/10 p-2">
              <div className="flex items-center gap-2 text-sm font-semibold text-amber-200">
                <Lock className="h-4 w-4" />
                {t('organizations.createRestricted')}
              </div>
              <p className="mt-3 text-sm leading-tight text-amber-100/85">{t('organizations.createRestrictedHint')}</p>
            </div>
          ) : (
            <div className="mt-3 rounded-[24px] border border-emerald-400/20 bg-emerald-400/10 p-2">
              <div className="text-sm font-semibold text-emerald-200">{t('organizations.createEnabled')}</div>
              <p className="mt-3 text-sm leading-tight text-emerald-100/85">{t('organizations.createEnabledHint')}</p>
            </div>
          )}
        </aside>
      </div>
