      ...
      <div className="max-w-6xl mx-auto space-y-3">
        <div className="flex justify-between items-center">
          ...
          {!isAdding && !editingId && (
            <button
              onClick={startAdding}
              className="px-3 py-2.5 bg-blue-600 text-white rounded-sm font-medium hover:bg-blue-700 transition-colors flex items-center shadow-sm"
            >
        ...
        {/* Edit / Add Form */}
        {(isAdding || editingId) && (
          <div className="bg-white p-1.5 rounded-sm border border-slate-200 shadow-sm animate-in fade-in slide-in-from-top-4">
            <h2 className="text-lg font-semibold text-slate-900 mb-3">{editingId ? 'Edit TM Entry' : 'Add New TM Entry'}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-1 mb-3">
              <div className="md:col-span-2 grid grid-cols-2 gap-1">
                 <div>
                   ...
                   <input
                     ...
                     className="w-full rounded-sm border border-slate-300 px-2 py-2 outline-none focus:border-blue-500 uppercase"
                   />
                 </div>
                 <div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full rounded-sm border border-slate-300 px-2 py-1.5 outline-none focus:border-blue-500 min-h-[100px]"
              ...
              <div className="md:col-span-2 w-1/3">
                ...
                <input
                  ...
                  className="w-full rounded-sm border border-slate-300 px-2 py-2 outline-none focus:border-blue-500"
                />
              </div>
            </div>

            <div className="flex justify-end gap-1 pt-4 border-t border-slate-100">
              <button
                onClick={resetForm}
                className="px-3 py-2 text-slate-700 bg-white border border-slate-300 rounded-sm font-medium hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="px-3 py-2 bg-blue-600 text-white rounded-sm font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center"
              >
        ...
        <div className="bg-white rounded-sm border border-slate-200 shadow-sm overflow-hidden">
          {isLoading ? (
            ...
            <div className="py-20 text-center text-slate-500">
              <Database className="w-12 h-8 mx-auto mb-2 text-slate-300" />
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="px-3 py-2 font-semibold text-slate-900 w-[40%]">Source Text</th>
                    <th className="px-3 py-2 font-semibold text-slate-900 w-[40%]">Target Text</th>
                    <th className="px-3 py-2 font-semibold text-slate-900 text-center w-auto">Quality</th>
                    <th className="px-3 py-2 font-semibold text-slate-900 w-[100px] text-right">Actions</th>
                  </tr>
                </thead>
                    ...
                    <tr key={entry.id} className="hover:bg-slate-50/50 transition-colors align-top">
                      <td className="px-3 py-2">
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm text-slate-900 whitespace-pre-wrap">{entry.sourceText}</span>
                        </div>
                      </td>
                      <td className="px-3 py-2">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold uppercase text-slate-400 mb-1 tracking-wider">{entry.targetLanguage}</span>
                          <span className="text-sm text-blue-700 whitespace-pre-wrap">{entry.targetText}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-sm transition-colors"
                            title="Edit"
                          >
